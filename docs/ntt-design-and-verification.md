...
| `ntt_core.sv` | **Increment 2 — functionally complete & verified** | Single-butterfly memory-based NTT/INTT + SCALE; control FSM mirrors the reference schedule. 33/33 golden-compared checks pass. Increment 3 widens to the pipelined 2×2 tile; OP_PWM is a later increment |
...
| 2026-05-23 | **Verification increment 4 — `ntt_engine` stream front-end complete & verified.** Implemented the AXI-Stream front-end FSM (RX 256 → RUN → TX 256) wrapping `ntt_core`, plus `tb_ntt_engine.sv` + `sim/ntt_engine_check.do`. **23/23 checks pass**. Two testbench handshake bugs found & fixed during bring-up (premature `m_tready` deassert; `start` pulsed during the 1-cycle `E_DONE` state) — RTL was correct, TB now waits on `busy_o`; a watchdog was added. Remaining: increment 3 (widen `ntt_core` to the pipelined conflict-free 2×2 tile) and OP_PWM (needs a two-operand interface). |
